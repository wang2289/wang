const util = require('../../utils/util.js')
import Dialog from 'vant-weapp/dialog/dialog';
Page({
  data: {
    carts: [], // 购物车列表
    hasList: false, // 列表是否有数据
    totalPrice: 0, // 总价，初始为0
    selectAllStatus: true, // 全选状态，默认全选
    noneSelectAllStatus: false, //全不选
    obj: {
      name: "hello"
    },
    selectindex: 100
  },
  bindselect: function(e) {
    this.setData({

      selectindex: e.currentTarget.dataset.index
    })
  },
  payclick: function() {
    var that = this;
    var cartslist = this.data.carts;

    var result = "";

    for (let i = 0, len = cartslist.length; i < len; i++) {
      if (cartslist[i].selected)
        result += cartslist[i].type + ':' + cartslist[i].id + ';'

    }
    console.log(result)
    result = result.substr(0, result.length - 1)
    console.log(result)
    wx.navigateTo({
      url: './orderpay/orderpay?result=' + result,
    })

  },



  onShow() {
    util.requestsendtoken('api/UserShopping/GetUserShoppingList?', null, {}, res => {
      console.log(res.data)
      this.setData({
        // hasList: true,
        carts: res.data
      });
      if (res.data.length) {
        this.setData({
          hasList: true,
          selectAllStatus: true
        });
        this.getTotalPrice();
      } else{
        this.setData({
          selectAllStatus:false
        })
      }

    })


  },
  /**
   * 当前商品选中事件
   */
  selectList(e) {
    const index = e.currentTarget.dataset.index;
    let carts = this.data.carts;
    const selected = carts[index].selected;
    carts[index].selected = !selected;
    // if (carts[index].selected == true){
    //   this.setData({
    //     noneSelectAllStatus: false
    //   })
    // }
    for (let i = 0; i < carts.length; i++){
      if (carts[i].selected == true){
        this.setData({
         noneSelectAllStatus: false
        })
      }
      if (carts[i].selected == false){
        this.setData({
          selectAllStatus: false
        })
      }
    }
    this.setData({
      carts: carts
    });
    this.getTotalPrice();
  },

  /**
   * 删除购物车当前商品
   */
  onClose(event) {
    const {
      position,
      instance
    } = event.detail;
    console.log(event)
    switch (position) {
      case 'left':
      case 'cell':
        instance.close();
        break;
      case 'right':
        Dialog.confirm({
          message: '确定删除吗？'
        }).then(() => {
          instance.close();
          this.deleteList(event);
        });
        break;
    }
  },


  deleteList(e) {

    console.log(e);
    const index = e.currentTarget.dataset.index;
    let carts = this.data.carts;
    var aomid = carts[index].id;

    util.requestsendtoken('api/UserShopping/DelUserShopping?', null, {
      aomid: aomid
    }, res => {

      if (res.code == 0) {
        carts.splice(index, 1);
        this.setData({
          carts: carts
        });
        if (!carts.length) {
          this.setData({
            hasList: false,
            totalPrice: 0
          });
        } else {
          this.getTotalPrice();
        }
      }
    })



  },

  /**
   * 购物车全选事件
   */
  selectAll(e) {
    let selectAllStatus = this.data.selectAllStatus;
    //selectAllStatus = !selectAllStatus;
    let carts = this.data.carts;

    for (let i = 0; i < carts.length; i++) {
      carts[i].selected = true;
    }
    this.setData({
      selectAllStatus: true,
      carts: carts,
      noneSelectAllStatus: false
    });
    this.getTotalPrice();
  },
  // 购物车全不选事件
  selectAllNone(e) {
    let noneSelectAllStatus = this.data.noneSelectAllStatus;
    let carts = this.data.carts
    for (let i = 0; i < carts.length; i++) {
      carts[i].selected = false;
    }
    this.setData({
      noneSelectAllStatus: true,
      selectAllStatus: false,
      carts: carts
    });
    this.getTotalPrice();
  },



  /**
   * 计算总价
   */
  getTotalPrice() {
    let carts = this.data.carts; // 获取购物车列表
    let total = 0;
    for (let i = 0; i < carts.length; i++) { // 循环列表得到每个数据
      if (carts[i].selected) { // 判断选中才会计算价格
        var money = Number(carts[i].money)
        total += money; // 所有价格加起来
      }
    }
    console.log(total)
    this.setData({ // 最后赋值到data中渲染到页面
      carts: carts,
      totalPrice: total.toFixed(2)
    });
  }

})